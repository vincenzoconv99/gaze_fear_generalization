...
dataset_name = 'Reutter'
lib = 'pymc'
method = 'SVI'
dataset_path = 'datasets/Reutter/'
...
def extract_features_sub(sub_data, sub, parameters, lib, method, dset):
    ...
    :return: None
    '''
    sub_paths = sorted(os.listdir(dataset_path))

    print('\nSubject number', sub+1)
    print('Subject identity ', sub_paths[sub])
        ...
        for fi, curr_fix in enumerate(all_fix):
            ...
            try:
                ...
                with model:
                    ...
                    approx = pm.fit(n=20000, method=pm.ADVI()) # find map
    ...
    save_event_features(all_features, dataset_name, "event_features_" + str(sub_paths[sub]), type='OU_posterior', method='VI', dset=dset)
...
def get_all_features(data):
    ...
    for sub, sub_data in enumerate(data):
        extract_features_sub(sub_data, sub, parameters, lib, method, dset='train')
