...
from sklearn.svm import LinearSVR, LinearSVC, OneClassSVM, SVR
...
def evaluate(clf_fix, clf_sac, X_fix_test, y_f_test, stim_f_test, sub_f_test, X_sac_test, y_s_test, stim_s_test, sub_s_test, regression=True):
    ...
    for i,k in enumerate(key_fix):
        ...
        y_test[i] = map_ss_rating[(subject, stimulus)]
...
def load_dataset(path, regression=True, nsub=None, num_sessions=None):
    ...
    for file in subs:
        ...
        fix_data = np.hstack([curr_subject_id_f, fix_rating_labels, stim_fix, fix_data])
        sac_data = np.hstack([curr_subject_id_s, sac_rating_labels, stim_sac, sac_data])

        fix_data = [x for x in fix_data  if np.isnan(x[1]) == False]
        sac_data = [x for x in sac_data  if np.isnan(x[1]) == False]
    ...
    if regression:
        print('std rating in Fixations', np.std([x[1] for x in data_fix]))
        print('std rating in Saccades', np.std([x[1] for x in data_sac]))
    else:
...
for regression, models in  [ (False, models_classification) , (True, models_regression), ]:
    ...
    map_ss_rating = {} # Mapping (subject, stimulus) to Rating
    for x in data_fix[:, :3]:
